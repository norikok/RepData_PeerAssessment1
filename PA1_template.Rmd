...
The data set was loaded at `r time`. It consists of `r nrows` rows corresponding to `r ndays` days and the following fields: `r names`.
...
The missing values (coded as `NA`) are ignored here and interpreted to mean that no measurements were taken (as opposed to step count being 0). The following dates do not have any valid data: `r invalid_dates`. Hence the histogram only represents the `r nvalid_dates` dates in the dataset that had at least one valid (not `NA`) value.  When considering only those dates:
...
The 5-minute interval with the largest number of steps on average was interval `r interval_max_steps` (i.e. period beginning at `r interval_hour`:`r interval_min`). 
...
- **mean** total number of steps taken per day is `r mean_filled_steps`.
- **median** total number of steps taken per day is `r median_filled_steps`.
